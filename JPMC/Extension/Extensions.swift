...
protocol PullableTableView  where Self : UITableView{
    
}
extension PullableTableView  {
...
extension NSLayoutConstraint {
    static func getSameFrameConstraint(subView :UIView , superView : UIView) -> [NSLayoutConstraint] {
...
extension UITableView{
    ...
    func register<Header>(header : Header.Type) where Header : Reusable_NibLoadable_ViewProtocol {
        register(UINib(nibName: header.nibName, bundle: nil), forHeaderFooterViewReuseIdentifier: header.reuseIdentifier)
    }
...
extension FileManager {
    static  func save<G: Encodable>(data: G, filePath: URL) {
        let encoder = JSONEncoder()
        do {
            let adata = try encoder.encode(data)
            if FileManager.default.fileExists(atPath: filePath.path) {
                try FileManager.default.removeItem(at: filePath)
            }
            FileManager.default.createFile(atPath: filePath.path, contents: adata, attributes: nil)
        } catch {
            print("error while saving the data in directory \(error.localizedDescription)")
        }
    }
    static  func retrieve<G: Decodable>(forFilePath filePath: URL) -> G? {
        ...
        if let data = FileManager.default.contents(atPath: filePath.path) {
            ...
            do {
                let model = try decoder.decode(G.self, from: data)
                return model
            } catch {
                print(error.localizedDescription)
            }
        } else {
            print("No data at file Path \(filePath.path)")
        }
        return nil
    }
}
